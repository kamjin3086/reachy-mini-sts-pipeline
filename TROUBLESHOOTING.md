...
| `Model 'X' not found` (llama-swap side) | Model not registered in llama-swap | Change `--model_name` in `sts_start.sh` to a registered model, or add to llama-swap `config.yaml` |
| `unable to start process: upstream command exited prematurely but successfully` | llama-swap server-side config issue | Check llama-swap logs; verify the model command is valid |
